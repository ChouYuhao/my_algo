...
class SACAgent:
    ...
    def set_rng_states(self, torch_rng_state, random_rng_state):
        ...
        # torch.set_rng_state(torch_rng_state.to(self.device))
    ...
    def save_checkpoint(self, env_name, suffix="", ckpt_path=None):
        ...
        if not os.path.exists('checkpoints/FourRooms/test'):
            os.makedirs('checkpoints/FourRooms/test')
        if ckpt_path is None:
            ckpt_path = "checkpoints/FourRooms/test/DIAYN_checkpoint_{}_{}".format(env_name, suffix)
            print(ckpt_path)
    ...
    def load_checkpoint(self, path):
        ...
        if self.config["unity_camera"]:
            ...
            self.ray.load_state_dict(checkpoint['Ray'])
