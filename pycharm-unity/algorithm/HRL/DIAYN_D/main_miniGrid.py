...
if __name__ == "__main__":
    ...
    if params["do_train_skill"]:
        ...
        # env = ActionBonus(env)
        env = RGBImgObsWrapper(env)  # obs.dim (40 * 40 * 3)

        tb_writer = SummaryWriter(log_dir="runs/DoorKey-5x5/imgae")
        ...
        for episode in range(1, params["max_episodes"]):
            ...
            obs, info = env.reset()
            if params["if_conv"]:
                obs = agent.con_states(obs, z, params["n_skills"])
            else:
            ...
            for step in range(env.max_steps):
                ...
                if not params["if_conv"]:
                    next_obs = concat_state_latent(next_obs['image'], z, params["n_skills"])
                else:
                    next_obs = agent.con_states(next_obs, z, params["n_skills"])
